...
def get_train_batches(generators,train_batch_size,noise, perturbp, resample=None):
    while True:
        ...
        if noise > 0:
            ...
            noisestd = noise/batches_snr[:,np.newaxis, np.newaxis]
            #noisestd = np.where(noisestd < noiseclip, noisestd, noiseclip)
            batches_x += noisestd * np.random.randn(shp0, shp1, shp2)
        if perturbp > 0:
            batches_x = perturb_batch(batches_x, p=perturbp) 
                
        # if resample is not None and np.random.random()>0.8:
        #     batches_x = resample(batches_x, f=resample)
...
def get_train_batches_small_memory(data_names,train_batch_size, number_of_epochs, tsteps_per_file, noise, perturbp, load_mods=None):
    """
