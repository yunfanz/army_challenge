...
parser.add_argument('--noise', type=float, default=.33)
...
#global conv_index
#conv_index=0
def inception(input_img, height = 1,widths=[9,4], fs=[64,64,64,64,64], with_residual=False, tw_tower=False):
...
def googleNet(x, data_format='channels_last', num_classes=24,num_layers=[1,2,4,2], features=[1,1,1,1,1]):
    ...
    for dep in range(num_layers[0]):
        y = x
        x = Conv2D(filters=192*features[1], kernel_size=[1, 4], strides=[1,1], padding='same', activation='relu')(x)
        if dep > 0 and args.res:
            x = add([x,y])
    x = MaxPooling2D([1,2], strides=[1,2], padding='same')(x)
    for dep in range(num_layers[1]):
        y = x
        x = inception(x, height=2,widths=[9,4], fs=np.array([32,32,32,32,32])*features[2], tw_tower=True)
        if dep > 0 and args.res:
            x = add([x,y])
    x = MaxPooling2D([1,2], strides=2, padding='same')(x)
    for dep in range(num_layers[2]):
        y = x
        x = inception(x, height=2,widths=[9,4], fs=np.array([48,96,48,96,96])*features[3], with_residual=True)
        if dep > 0 and args.res:
            x = add([x,y])
    x = MaxPooling2D([1,2], strides=2, padding='same')(x)
    x = Dropout(0.1)(x)
    for dep in range(num_layers[3]):
        y = x
        x = inception(x, height=1,widths=[9,4],fs=np.array([64,64,64,64,64])*features[4])
        if dep > 0 and args.res:
            x = add([x,y])
    out = out_tower(x, dr=0.55)
...
def boost_generator(batch_size):
    data = np.load('misclassified_training/training_data15.npy')
    labels = np.load('misclassified_training/training_labels15.npy')
    ...
    while(True):
        ...
        yield data[beg:end], labels[beg:end],[1]*batch_size
        beg = end
        

if args.boosting:
    generators.append(boost_generator(train_batch_size))
    tsteps += 288000
...
def get_train_batches(generators):
    while True:
        ...
        if args.perturb > 0:
            sizes = np.random.randint(900,1024, batches_x.shape[0])
            perturb = np.random.uniform(0,1,batches_x.shape[0])
            for i,sig in enumerate(batches_x):
                if perturb[i] < args.perturb:
                    sig = sig[0] + 1j * sig[1]
                    beg = np.random.randint(0, 1024-sizes[i])
                    sig = sig[beg:beg+sizes[i]]
                    sig = resample(sig, 1024)
                    batches_x[i][0] = sig.real
                    batches_x[i][1] = sig.imag
